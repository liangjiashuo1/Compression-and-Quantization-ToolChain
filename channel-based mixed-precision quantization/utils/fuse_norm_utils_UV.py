...
def get_first_linear(layer, name):
    """
    Return the first linear layer of a projection.
    """
    if getattr(layer, f"{name}_lowrank"):
        return getattr(layer, f"{name}_proj_1")
    else:
        return getattr(layer, f"{name}_proj")
...
def fuse_layer_norms(model):
    ...
    for layer in layers:
        ...
        fuse_ln_linear(
            ...
            [
                get_first_linear(attn, "q"),
                get_first_linear(attn, "k"),
                get_first_linear(attn, "v"),
            ],
        )
