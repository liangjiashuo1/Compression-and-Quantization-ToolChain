...
from utils import fuse_norm_utils_UV, hadamard_utils, quant_utils, utils
...
def prepare_model(args, model):
    ...
    # Rotate the weights
    # 这里把 ln 乘到后面的 linear 权重中，将 ln 权重全部赋为 1，所以这里的 ln 不会影响权重
    fuse_norm_utils_UV.fuse_layer_norms(model)
    #对 down 层权重做 hadamard 旋转
    apply_r3_r4.rotate_model(model, args)
    utils.cleanup_memory(verbos=True)

    # 用于激活值量化，把对应 module 转换为 quant 单元，进行量化
    ...
    if args.a_bits < 16 or args.v_bits < 16:
        ...
        for name in qlayers:
            ...
            if "v_proj" in name and "v_proj_1" not in name and args.v_bits < 16:  # Set the v_proj precision
            ...
            if "down_proj" in name and "down_proj_2" not in name:  # Set the down_proj precision
    ...
    if args.k_bits < 16:
        if args.k_pre_rope:
            ...
            for layer in layers:
                # 在 attention 的 forward 中 apply_rotary_pos_emb 后对 q、k 的值做旋转并量化
