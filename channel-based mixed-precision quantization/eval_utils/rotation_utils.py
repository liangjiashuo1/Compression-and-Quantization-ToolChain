...
def get_input_linear(attn, name):
    if getattr(attn, f"{name}_lowrank"):
        return getattr(attn, f"{name}_proj_1")
    else:
        return getattr(attn, f"{name}_proj")
...
def rotate_attention_inputs(layer, R1) -> None:
    ...
    linears = [
        attn.q_proj_1 if attn.q_lowrank else attn.q_proj,
        attn.k_proj_1 if attn.k_lowrank else attn.k_proj,
        attn.v_proj_1 if attn.v_lowrank else attn.v_proj,
    ]
...
def rotate_attention_output(layer, R1) -> None:
    ...
    attn = layer.self_attn 
    W = attn.o_proj_2 if attn.o_lowrank else attn.o_proj
...
def rotate_mlp_input(layer, R1):
    ...
    up_linear = mlp.up_proj_1 if mlp.up_lowrank else mlp.up_proj
    gate_linear = mlp.gate_proj_1 if mlp.gate_lowrank else mlp.gate_proj

    mlp_inputs = [up_linear, gate_linear]
...
def rotate_mlp_output(layer, R1):
    ...
    if mlp.down_lowrank:
        # ----- LowRank case -----
        W2 = mlp.down_proj_2
        dtype = W2.weight.data.dtype
        W_ = W2.weight.data.to(device="cuda", dtype=torch.float64)

        W2.weight.data = torch.matmul(R1.T, W_).to(device="cpu", dtype=dtype)

        # hadamard apply on first projection
        W1 = mlp.down_proj_1
        apply_exact_had_to_linear(
            W1, had_dim=-1, output=False
        )

        if W2.bias is not None:
            b = W2.bias.data.to(device="cuda", dtype=torch.float64)
            W2.bias.data = torch.matmul(R1.T, b).to(device="cpu", dtype=dtype)
...
def rotate_ov_proj(layer, head_num, head_dim, R2=None):
    ...
    # V projection (output side)
    v_proj = attn.v_proj_2 if attn.v_lowrank else attn.v_proj

    # O projection (input side)
    o_proj = attn.o_proj_1 if attn.o_lowrank else attn.o_proj
