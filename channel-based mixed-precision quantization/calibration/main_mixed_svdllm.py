...
def _load_mmlu_text_corpus_svdllm(cache_dir=None):
...
def _load_wikitext_text_corpus(cache_dir=None):
    traindata = data_utils.datasets.load_dataset(
        "Salesforce/wikitext",
        "wikitext-2-raw-v1",
        split="train",
        cache_dir=cache_dir,
    )
    texts = [text for text in traindata["text"] if text and text.strip()]
    return "\n\n".join(texts)


def _sample_sequences_from_corpus(tot_text, tokenizer, nsamples, seqlen, rng):
    samples = []
...
def get_mixed_calibration_loader_svdllm(
    nsamples=128,
    seed=0,
    seqlen=2048,
    model="",
    ...
    mmlu_count = int(round(nsamples * MMLU_RATIO))
    wiki_count = nsamples - mmlu_count

    trainloader = []

    if mmlu_count > 0:
        mmlu_rng = random.Random(rng.randint(0, 2**31 - 1))
        mmlu_corpus = _load_mmlu_text_corpus_svdllm(cache_dir=cache_dir)
        trainloader.extend(
    ...
    if wiki_count > 0:
        wiki_rng = random.Random(rng.randint(0, 2**31 - 1))
        wiki_corpus = _load_wikitext_text_corpus(cache_dir=cache_dir)
        trainloader.extend(
            _sample_sequences_from_corpus(
                wiki_corpus,
                tokenizer,
...
def ptq_model(args, model, model_args=None):
    ...
    if args.w_bits < 16:
        ...
        if args.load_qmodel_path:
            ...
            trainloader = get_mixed_calibration_loader_svdllm(
